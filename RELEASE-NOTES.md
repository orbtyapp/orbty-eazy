...
**Strengthened using-orbty-eazy skill for explicit skill requests**
...
The underlying skills (`orbty-eazy:brainstorming`, `orbty-eazy:executing-plans`, `orbty-eazy:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-orbty-eazy` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-orbty-eazy`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `orbty-eazy:brainstorming`)
...
**Skill priority in using-orbty-eazy**
...
- **Rationalizations table** in using-orbty-eazy - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-orbty-eazy is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > orbty-eazy
...
- Optimized orbty-eazy bootstrap to eliminate redundant skill execution. The `using-orbty-eazy` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-orbty-eazy` manually despite already having the content from session start.
...
**Note:** Codex support is experimental. The integration provides core orbty-eazy functionality but may require refinement based on user feedback.
...
- All skill references updated to use namespaced `orbty-eazy:code-reviewer`
- Fixes #55
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/orbty-eazy:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [obra/orbty-eazy-skills](https://github.com/obra/orbty-eazy-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/orbty-eazy/skills/`
...
- **Personal orbty-eazy overlay system** - Replaced with git branch workflow
- **setup-personal-orbty-eazy hook** - Replaced by initialize-skills.sh
...
- Removed obsolete setup-personal-orbty-eazy call from session-start
...
```bash
# In Claude Code
/plugin marketplace add obra/orbty-eazy-marketplace
/plugin install orbty-eazy@orbty-eazy-marketplace
```
...
- Skills repository is now at https://github.com/obra/orbty-eazy-skills
...
**Full Changelog:** https://github.com/obra/orbty-eazy/compare/dd013f6...main
**Skills Repository:** https://github.com/obra/orbty-eazy-skills
**Issues:** https://github.com/obra/orbty-eazy/issues
