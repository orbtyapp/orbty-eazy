...
export const SuperpowersPlugin = async ({ client, directory }) => {
  ...
  // Derive orbty-eazy skills dir from plugin location (works for both symlinked and local installs)
  const orbty-eazySkillsDir = path.resolve(__dirname, '../../skills');
  const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

  // Helper to generate bootstrap content
  const getBootstrapContent = (compact = false) => {
    const usingSuperpowersPath = skillsCore.resolveSkillPath('using-orbty-eazy', orbty-eazySkillsDir, personalSkillsDir);
...
**Skills naming (priority order):** project: > personal > orbty-eazy:`
...
**IMPORTANT: The using-orbty-eazy skill content is included below. It is ALREADY LOADED - you are currently following it. Do NOT use the use_skill tool to load "using-orbty-eazy" - that would be redundant. Use use_skill only for OTHER skills.**
  ...
  return {
    tool: {
      use_skill: tool({
        ...
        args: {
          skill_name: tool.schema.string().describe('Name of the skill to load (e.g., "orbty-eazy:brainstorming", "my-custom-skill", or "project:my-skill")')
        },
        execute: async (args, context) => {
          const { skill_name } = args;

          // Resolve with priority: project > personal > orbty-eazy
          ...
          if (forceProject || !skill_name.startsWith('orbty-eazy:')) {
          ...
          // Fall back to personal/orbty-eazy resolution
          if (!resolved && !forceProject) {
            resolved = skillsCore.resolveSkillPath(skill_name, orbty-eazySkillsDir, personalSkillsDir);
          }
...
# ============================================`;
      ...
      find_skills: tool({
        description: 'List all available skills in the project, personal, and orbty-eazy skill libraries.',
        args: {},
        execute: async (args, context) => {
          ...
          const orbty-eazySkills = skillsCore.findSkillsInDir(orbty-eazySkillsDir, 'orbty-eazy', 3);

          // Priority: project > personal > orbty-eazy
          const allSkills = [...projectSkills, ...personalSkills, ...orbty-eazySkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install orbty-eazy skills to ~/.config/opencode/orbty-eazy/skills/ or add project skills to .opencode/skills/';
          }
