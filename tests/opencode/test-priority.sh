...
# Verifies that skills are resolved with correct priority: project > personal > orbty-eazy
...
# 1. Create in orbty-eazy location (lowest priority)
mkdir -p "$HOME/.config/opencode/orbty-eazy/skills/priority-test"
cat > "$HOME/.config/opencode/orbty-eazy/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Orbty-eazy version of priority test skill
---
# Priority Test Skill (Orbty-eazy Version)

This is the ORBTY-EAZY version of the priority test skill.
...
if [ -f "$HOME/.config/opencode/orbty-eazy/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Orbty-eazy version exists"
else
    echo "  [FAIL] Orbty-eazy version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides orbty-eazy
echo ""
echo "Test 2: Testing personal > orbty-eazy priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides orbty-eazy)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [FAIL] Orbty-eazy version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|orbty-eazy" | head -10
fi

# Test 3: Test that project overrides both personal and orbty-eazy
echo ""
echo "Test 3: Testing project > personal > orbty-eazy priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Orbty-eazy version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit orbty-eazy: prefix bypasses priority
echo ""
echo "Test 4: Testing orbty-eazy: prefix forces orbty-eazy version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load orbty-eazy:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [PASS] orbty-eazy: prefix correctly forces orbty-eazy version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] orbty-eazy: prefix did not force orbty-eazy version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
